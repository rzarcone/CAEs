...
class EntTest(tf.test.TestCase):
    ...
    def test_integral_end_point(self):
        ...
        for run in range(num_runs):
            ...
            end_2 = np.zeros(1,dtype=np.float32)
            ...
            rand_ep = np.where(np.logical_and(rand_ms == 0, rand_bs == 0), np.zeros_like(rand_ys, dtype=np.float32), 
                np.where(np.logical_and(rand_ms == 0, np.logical_not(rand_bs == 0)), -rand_xs*rand_bs*np.log(rand_bs), 
                np.divide(rand_ms*rand_xs*(rand_ms*rand_xs+2*rand_bs)-2*ylogy, 4*rand_ms, dtype=np.float32)))
            ## Add GT to inputs
            ...
            for x_points, y_points, ms_gt, bs_gt, ep_gt in inputs:
                with self.test_session() as sess:
                    ...
                    tf.global_variables_initializer().run()
                    self.assertAllClose(ep_gt,ep.eval())
        _ = np.seterr(**old_err)
